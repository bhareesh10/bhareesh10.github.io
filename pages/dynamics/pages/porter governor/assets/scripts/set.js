...
function mydata() {
  ...
  let a = [];
  for (let i = 0; i < 5; i++){
    a[i] = (Math.acos(h[i]/l)).toFixed(2);
    }
  let r = [];
  for (let i = 0; i < 5; i++){
    r[i] = 50 + l*Math.sin(a[i]);
    }
  let c = [];
  for (let i = 0; i < 5; i++){
  ...
  document.getElementById(`h${i+1}`).innerHTML = h[i];
  document.getElementById(`a${i+1}`).innerHTML = (a[i]*57.29).toFixed(2);
